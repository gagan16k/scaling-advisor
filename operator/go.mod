...
replace (
	...
	github.com/gardener/scaling-advisor/planner => ../planner
	github.com/gardener/scaling-advisor/pricing => ../pricing
	github.com/gardener/scaling-advisor/samples => ../samples
)

// Pin k8s.io minor versions to match the planner stack (mirrors service/go.mod).
// Required because planner pulls in kube-scheduler internals that demand a
// consistent staging-repo version set.
replace (
